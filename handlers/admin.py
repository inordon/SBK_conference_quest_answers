...
async def handle_admin_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ВОПРОСЫ И ОЦЕНКИ (должны быть ПЕРВЫМИ)
    ...
    try:
        ...
        with get_session() as session:
            ...
            await update.message.reply_text(
                ...
                f"Пользователи могут начать задавать вопросы.",
                reply_markup=get_back_button("events_menu")
            )
...
async def list_events_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_session() as session:
        ...
        for event in events:
            ...
            message += (
                f"{status_emoji} <b>#{event.id}</b> {event.name}\n"
                f"   💬 Вопросов: {feedback_count} | "
                f"⭐ Оценок: {rating_count} ({avg_rating})\n"
...
async def close_event_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Выбор мероприятия для закрытия"""
    query = update.callback_query
    ...
    with get_session() as session:
        ...
        if not active_events:
            await query.edit_message_text(
                "ℹ️ Нет активных мероприятий для закрытия.",
                reply_markup=get_back_button("events_menu")
            )
            return
...
async def close_event_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE, event_id: int):
    """Подтверждение закрытия мероприятия"""
    query = update.callback_query
...
async def close_event_execute(update: Update, context: ContextTypes.DEFAULT_TYPE, event_id: int):
    """Выполнение закрытия мероприятия"""
    query = update.callback_query
            ...
            try:
                await context.bot.send_message(
                    ...
                    text=f"🔒 Сбор вопросов завершен!\n\n"
                         f"📊 Всего вопросов: {len(event.feedbacks)}"
                )
            except Exception as e:
                logger.warning(f"Не удалось отправить уведомление: {e}")
        
        await request_ratings_for_event(context, event)
        
        await query.edit_message_text(
            f"✅ Мероприятие закрыто!\n\n"
            f"📅 {event.name}\n"
            f"💬 Вопросов: {len(event.feedbacks)}\n\n"
...
async def close_all_events_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Подтверждение закрытия всех мероприятий"""
    query = update.callback_query
    ...
    with get_session() as session:
        ...
        await query.edit_message_text(
            ...
            f"Активных мероприятий: {active_count}\n\n"
...
async def close_all_events_execute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Выполнение закрытия всех мероприятий"""
    query = update.callback_query
...
async def request_ratings_for_event(context: ContextTypes.DEFAULT_TYPE, event: Event):
    """Запросить оценки у пользователей"""
        ...
        for user_id in user_ids:
            ...
            try:
                await context.bot.send_message(
                    ...
                    text=f"📊 Мероприятие \"{event.name}\" завершено!\n\n"
                         f"Пожалуйста, оцените его:",
                    reply_markup=get_rating_keyboard(event.id)
                )
            except Exception as e:
...
async def list_users_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_session() as session:
        ...
        message += f"👤 <b>Обычные пользователи:</b> {total_users} чел.\n"
        if users:
            message += "(показаны первые 50)\n"
...
async def handle_add_admin_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка ввода данных для добавления админа"""
...
async def add_admin_execute(update: Update, context: ContextTypes.DEFAULT_TYPE, identifier: str):
    """Выполнение добавления администратора"""
    with get_session() as session:
        user = None
        ...
        logger.info(f"Добавлен администратор: {user.telegram_id}")
...
async def handle_add_manager_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка ввода данных для добавления менеджера"""
...
async def add_manager_execute(update: Update, context: ContextTypes.DEFAULT_TYPE, identifier: str):
    """Выполнение добавления менеджера"""
    with get_session() as session:
        user = None
        ...
        logger.info(f"Добавлен менеджер: {user.telegram_id}")
...
async def handle_remove_role_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка ввода данных для снятия роли"""
        ...
        try:
            await context.bot.send_message(
                ...
                text=f"ℹ️ С вас снята роль {old_role}.\n"
                     f"Теперь у вас права обычного пользователя."
            )
        except:
            pass
        
        logger.info(f"Снята роль с пользователя: {user.telegram_id}")

@admin_only
async def promote_from_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Назначить менеджером через reply в рабочей группе"""
    if update.effective_chat.id != Config.WORK_GROUP_ID:
        return
    
    if not update.message.reply_to_message:
        await update.message.reply_text(
            "❌ Ответьте на сообщение пользователя, которого хотите назначить менеджером."
        )
        return
    ...
    with get_session() as session:
        ...
        if not user:
            ...
            if user.role == UserRole.MANAGER:
                await update.message.reply_text(
                    f"ℹ️ {user.full_name or user.username} уже является менеджером."
                )
                return
        ...
        await update.message.reply_text(
            f"✅ {user_display} назначен менеджером!"
        )
        ...
        logger.info(f"Менеджер назначен через группу: {user.telegram_id}")
...
async def export_report_select_event(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Выбор мероприятия для экспорта отчета"""
    query = update.callback_query
    ...
    with get_session() as session:
        ...
        if not events:
            await query.edit_message_text(
                "ℹ️ Нет завершенных мероприятий для отчета.",
                reply_markup=get_back_button("stats_menu")
            )
            return
        
        await query.edit_message_text(
            "📄 Выберите мероприятие для отчета:",
...
async def export_report_event(update: Update, context: ContextTypes.DEFAULT_TYPE, event_id: int):
    ...
    await query.edit_message_text(f"⏳ Генерирую отчет по мероприятию \"{event.name}\"...")
        ...
        with get_session() as session:
            ...
            with open(pdf_path, 'rb') as pdf_file:
                await context.bot.send_document(
                    ...
                    caption=f"📊 Отчет по мероприятию: {event.name}"
                )
...
async def edit_no_events_message_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Начать редактирование сообщения при отсутствии мероприятий"""
    query = update.callback_query
...
async def handle_no_events_message_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_session() as session:
        telegram_id = update.effective_user.id
        admin_user = session.query(User).filter_by(telegram_id=telegram_id).first()
        set_setting('no_events_message', new_message, admin_user.id)
